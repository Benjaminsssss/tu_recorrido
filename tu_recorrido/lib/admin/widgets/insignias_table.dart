...
class _InsigniasTableState extends State<InsigniasTable> {
  ...
  Future<void> _load() async {
    // debug
    // ignore: avoid_print
    print('InsigniasTable._load: starting');
    ...
    try {
      ...
      // ignore: avoid_print
      print(
          'InsigniasTable._load: fetched ${_insignias.length} insignias, ${_estacionPorInsignia.length} estaciones mapeadas');
    } catch (e) {
      // debug
      // ignore: avoid_print
      print('InsigniasTable._load: error -> $e');
      // show snackbar after frame to avoid inherited widget lookup during init
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error cargando insignias: $e')),
          );
        }
      });
      _insignias = [];
          ...
          ElevatedButton(
              onPressed: () async {
                ...
                if (kIsWeb) {
                  ...
                  await InsigniaService.createInsigniaWithImage(
                      imageFile: file, nombre: nombre, descripcion: desc);
                }
                await _load();
              },
  ...
  Future<void> _asignarInsignia(String insigniaId) async {
    ...
    if (estaciones.isEmpty) {
      await showDialog(
          ...
          builder: (ctx) => AlertDialog(
                  title: const Text('Asignar insignia'),
                  content:
                      const Text('No hay estaciones disponibles sin insignia.'),
                  actions: [
                    TextButton(
    ...
    await showDialog(
        context: context,
        builder: (ctx) => StatefulBuilder(builder: (dialogCtx, setStateSB) {
              return AlertDialog(
                  title: const Text('Asignar a estación'),
                  content: SizedBox(
                      ...
                      child: ListView.builder(
                          ...
                          itemBuilder: (c, i) {
                            ...
                            final selected = estacionSeleccionadaId == e.id;
                            return ListTile(
                              title: Text(e.nombre),
  ...
  Future<void> _otorgarInsignia(String insigniaId) async {
    ...
    await showDialog(
        context: context,
        builder: (dialogContext) => AlertDialog(
                title: const Text('Otorgar insignia'),
                content: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  TextField(
                      ...
                      decoration:
                          const InputDecoration(labelText: 'UID (opcional)'))
                ]),
                actions: [
                  TextButton(
                      onPressed: () => Navigator.of(dialogContext).pop(),
                  ...
                  ElevatedButton(
                      onPressed: () async {
                        ...
                        if (targetUid == null && email.isNotEmpty) {
                          ...
                          if (query.docs.isEmpty) {
                            ...
                            messenger.showSnackBar(const SnackBar(
                                content:
                                    Text('Usuario no encontrado por email')));
                            return;
                          }
                        ...
                        try {
                          ...
                          if (!mounted) return;
                          messenger.showSnackBar(const SnackBar(
                              content: Text('Insignia otorgada')));
                        } catch (e) {
                          if (!mounted) return;
                          messenger.showSnackBar(SnackBar(
                              content: Text('Error al otorgar insignia: $e')));
                        }
                      },
  ...
  Future<void> _editarInsignia(Insignia ins) async {
    ...
    await showDialog(
        ...
        builder: (ctx) => StatefulBuilder(builder: (context, setStateSB) {
              return AlertDialog(
                  title: const Text('Editar insignia'),
                  content: Column(mainAxisSize: MainAxisSize.min, children: [
                    TextField(
                          ...
                          try {
                            ...
                            if (picked != null) {
                              if (kIsWeb) {
                                final bytes = await picked!.readAsBytes();
                                final path =
                                    'insignias/${ins.id}_${picked!.name}';
                                final url = await StorageService.instance
                                    .uploadBytes(bytes, path,
                                        contentType: 'image/jpeg');
                                changes['imagenUrl'] = url;
                              } else {
                                final file = File(picked!.path);
                                final ext = file.path.split('.').last;
                                final path = 'insignias/${ins.id}.$ext';
                                final url = await StorageService.instance
                            ...
                            await InsigniaService.actualizarInsignia(
                                ins.id, changes);
                            await _load();
                          } catch (e) {
                            if (!mounted) return;
                            messenger.showSnackBar(SnackBar(
                                content: Text('Error editando insignia: $e')));
                          }
                        },
      ...
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 8),
            const Text('Gestión de Insignias',
          ...
          child: DataTable(
            ...
            rows: _insignias.map((ins) {
              ...
              return DataRow(cells: [
                DataCell(SizedBox(
                    width: 200,
                    child: Text(ins.nombre, overflow: TextOverflow.ellipsis))),
                DataCell(SizedBox(
                    width: 350,
                    child: Text(ins.descripcion,
                        overflow: TextOverflow.ellipsis))),
                DataCell(ClipRRect(
                    borderRadius: BorderRadius.circular(6),
                    child: Image.network(ins.imagenUrl,
                        width: 80,
                        height: 48,
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => Container(
                            width: 80, height: 48, color: Colors.grey)))),
                DataCell(Text(assigned?.nombre ?? '-')),
                DataCell(Row(children: [
                  if (assigned == null)
                    IconButton(
                        onPressed: () => _asignarInsignia(ins.id),
                        icon: const Icon(Icons.link),
                        tooltip: 'Asignar a estación'),
                  IconButton(
                      onPressed: () => _otorgarInsignia(ins.id),
                      icon: const Icon(Icons.emoji_events),
                      tooltip: 'Otorgar a usuario'),
                  IconButton(
                      onPressed: () => _editarInsignia(ins),
