...
class QRManagementHelper {
    ...
    try {
      ...
      for (final estacion in estaciones) {
        try {
          ...
          if (estacion.codigoQR.isNotEmpty && QRService.esCodigoValido(estacion.codigoQR)) {
            AppLogger.info('${estacion.nombre} - Ya tiene QR válido: ${estacion.codigoQR}');
            yaConQR++;
            continue;
          }
          
          // Generar nuevo código QR
          final nuevoCodigoQR = QRService.generarCodigoQR(estacion.id, estacion.nombre);
          ...
          AppLogger.info('${estacion.nombre} - Nuevo QR generado: $nuevoCodigoQR');
          actualizadas++;
    ...
    try {
      ...
      for (final estacion in estaciones) {
        if (estacion.codigoQR.isEmpty) {
          ...
          AppLogger.info('${estacion.nombre} - QR válido: ${estacion.codigoQR}');
          conQRValido++;
        } else {
          AppLogger.warning('${estacion.nombre} - QR inválido: ${estacion.codigoQR}');
          conQRInvalido++;
        }
      }
  ...
  static Future<Map<String, dynamic>> ejecutarDesdeAdmin() async {
    try {
      ...
      for (final estacion in estaciones) {
        if (estacion.codigoQR.isEmpty || !QRService.esCodigoValido(estacion.codigoQR)) {
          final nuevoCodigoQR = QRService.generarCodigoQR(estacion.id, estacion.nombre);
