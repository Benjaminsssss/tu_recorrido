...
class UserRoleService {
  ...
  static Future<bool> hasPermission(
      bool Function(UserPermissions) permission) async {
    final user = await getCurrentUser();
  ...
  static Future<List<AppUser>> getAllUsers() async {
    try {
      final snapshot =
          await _firestore.collection(_usersCollection).orderBy('nombre').get();
  ...
  static Future<void> promoteToAdmin(String uid) async {
    ...
    if (currentUser?.role.isAdmin != true) {
      throw Exception(
          'Solo los super administradores pueden promover usuarios');
    }
