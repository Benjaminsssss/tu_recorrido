...
class AlbumMigrationService {
  ...
  static Future<Map<String, dynamic>> diagnoseAlbum() async {
    ...
    try {
      ...
      if (currentUser != null) {
        try {
          ...
          result['firebasePhotos'] = {
            ...
            'items': firebasePhotos.docs.map((doc) => {
              'id': doc.id,
              'data': doc.data(),
            }).toList(),
          };
      ...
      final sharedPrefsCount = result['sharedPrefsPhotos']['count'] as int? ?? 0;
  ...
  static String formatDiagnosis(Map<String, dynamic> diagnosis) {
    ...
    final sharedPrefsPhotos = diagnosis['sharedPrefsPhotos'] as Map<String, dynamic>?;
    if (sharedPrefsPhotos != null) {
      ...
      if (sharedPrefsPhotos.containsKey('error')) {
        ...
        if (sharedPrefsPhotos['count'] > 0) {
          ...
          for (final item in items.take(5)) { // Mostrar solo las primeras 5
            buffer.writeln('    • ${item['id']} (${item['parentId']}) - ${item['title']}');
          }
          if (items.length > 5) {
      ...
      if (firebasePhotos.containsKey('error')) {
        ...
        if (firebasePhotos['count'] > 0) {
          ...
          for (final item in items.take(5)) { // Mostrar solo las primeras 5
            ...
            buffer.writeln('    • ${item['id']} (${data['badgeId']}) - ${data['description'] ?? 'Sin descripción'}');
          }
          if (items.length > 5) {
            buffer.writeln('    • ... y ${items.length - 5} más');
          }
    ...
    if (migrationInfo != null) {
      ...
      buffer.writeln('  - Migración necesaria: ${migrationNeeded ? 'SÍ' : 'NO'}');
      buffer.writeln('  - SharedPrefs: ${migrationInfo['sharedPrefsPhotos']} fotos');
